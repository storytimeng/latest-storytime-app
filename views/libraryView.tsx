    ...
    <div className="min-h-screen bg-accent-shade-1 max-w-[28rem] mx-auto">
      ...
      <div className="px-4 mb-6">
        <div className="relative">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-primary-shade-3" />
          <input
            type="text"
        ...
        <div className="space-y-4">
          {filteredItems.map((item) => (
            <Link href={`/story/${item.id}`} key={item.id}>
              <div className="flex gap-3 p-3 bg-universal-white rounded-lg border border-light-grey-2 hover:shadow-sm transition-shadow">
                {/* Cover Image */}
                <div className="relative w-16 h-20 flex-shrink-0 rounded-lg overflow-hidden bg-light-grey-2">
                  <Image
                    src={item.coverImage}
                    alt={item.title}
                    fill
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="space-y-2">
                    ...
                    {item.progress !== undefined && (
                      <div className="w-full bg-light-grey-2 rounded-full h-1">
                        <div
                          className="bg-complimentary-colour h-1 rounded-full transition-all duration-300"
